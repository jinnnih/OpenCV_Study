...
def find_license_plate(img):
    ...
    # 가로선 강조 (번호판은 가로 직사각형)
    # 수정 전: kernel = cv.getStructuringElement(cv.MORPH_RECT, (20, 5))
    # 수정 후: 가로로 더 묵직하게 뭉치도록 변경
    ...
    for cnt in contours:
        ...
        if 35000> area > 900:  # 최소 면적
            x, y, w, h = cv.boundingRect(cnt)
            aspect_ratio = w / h  # 가로세로 비율
            
            # 번호판: 가로가 세로의 3~6배
            # 수정 전: if 3 < aspect_ratio < 6:
            # 수정 후: 번호판이 약간 대각선이어도 잡히도록 범위를 넓힘
...
img = cv.imread('car2.png')

if img is None:
    print("❌ 이미지를 불러올 수 없습니다. 'car2.png' 파일을 확인하세요.")

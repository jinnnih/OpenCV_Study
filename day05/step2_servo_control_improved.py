...
while(1):
    ...
    hsv = cv.cvtColor(frame, cv.COLOR_BGR2HSV)
    # 파란색 마스크
    mask_blue = cv.inRange(hsv, np.array([100, 100, 100]), np.array([130, 255, 255]))
    # 노란색 마스크 (예시)
    mask_yellow = cv.inRange(hsv, np.array([20, 100, 100]), np.array([30, 255, 255]))
    # 두 마스크 합치기 (OR 연산)
    combined_mask = cv.bitwise_or(mask_blue, mask_yellow)

    # 면적 계산
    pixel_count = cv.countNonZero(combined_mask)
    is_detected = pixel_count > 500
    area = detect_color(frame)
    ...
    # --- [화면 표시] FPS 및 상태 ---
    cv.putText(frame, f"FPS: {int(fps)}", (10, 60), cv.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
